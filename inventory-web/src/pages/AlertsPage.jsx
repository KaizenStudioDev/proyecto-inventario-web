  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6 animate-slide-up">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-2">Stock Alerts</h1>
          <p className="text-gray-600">Monitor low stock and out-of-stock items requiring action</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="card border-l-4 border-red-600">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-4xl font-bold text-gray-900">{outOfStock.length}</p>
            ...
            <div className="card border-l-4 border-amber-600">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs uppercase tracking-wide text-gray-500 mb-1">Warning</p>
                  <p className="text-4xl font-bold text-gray-900">{lowStock.length}</p>
          ...
          {/* Out of Stock */}
          <div className="card border border-red-200 bg-white">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 bg-red-50 rounded-md flex items-center justify-center border border-red-200">
                <span className="text-red-700 text-sm font-semibold">Alert</span>
              </div>
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Out of Stock</h2>
                <p className="text-xs text-gray-600">{outOfStock.length} products need immediate attention</p>
              </div>
            </div>
            {outOfStock.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {outOfStock.map(product => (
                  <div key={product.id} className="bg-white border-2 border-red-200 rounded-md p-4">
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="font-semibold text-gray-900 text-sm">{product.name}</h3>
                      <span className="badge badge-danger">OUT</span>
                    </div>
                    <p className="text-xs text-gray-600 mb-2">SKU: {product.sku}</p>
                    <p className="text-xs text-gray-600 mb-3">Price: {formatCurrency(product.unit_price)}</p>
                    <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                      <span className="text-xs font-semibold text-red-700">Stock: {product.stock}</span>
                      <span className="text-xs text-gray-600">Min: {product.min_stock}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="card border border-amber-200 bg-white">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 bg-amber-50 rounded-md flex items-center justify-center border border-amber-200">
                <span className="text-amber-700 text-sm font-semibold">Alert</span>
              </div>
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Low Stock</h2>
                <p className="text-xs text-gray-600">{lowStock.length} products running low</p>
              </div>
            </div>
            {lowStock.length === 0 ? (
              <div className="text-center py-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {lowStock.map(product => (
                  <div key={product.id} className="bg-white border-2 border-amber-200 rounded-md p-4">
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="font-semibold text-gray-900 text-sm">{product.name}</h3>
                    ...
                    <p className="text-xs text-gray-600 mb-3">Price: {formatCurrency(product.unit_price)}</p>
                    <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                      <span className="text-xs font-semibold text-amber-700">Stock: {product.stock}</span>
                      <span className="text-xs text-gray-600">Min: {product.min_stock}</span>
                    </div>
                  </div>
                ))}
